...
def search(gitlab_server, token, file_filter, text, group=None, project_filter=None, api_debug=False,
           internal_debug=False, filename_regex=False):
    return_value = []
    ...
    filter_groups = not (group == '' or group is None)
    filter_projects = not (project_filter == '' or project_filter is None)
    ...
    for project in projects:
        ...
        for file in files:
            if internal_debug:
                file_path = file.get('path', None) if file.get('path', None) is not None else file.get('name', None)
                eprint("  File: ", file_path)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "--filename-is-regex",
        action="store_true",
        help="FILE_FILTER become Python regular expressions,"
             "so it can be '.*.cpp' to search for all files with extension cpp"
    )
    ...
    parser.add_argument("GROUP", nargs='?',
                        help="Group to search for projects in, can be subgroup eg. "
                             "parent_group/subgroup/another_subgroup")
    ...
    group_arg = None if args.GROUP is None else args.GROUP
    project_filter_arg = None if args.PROJECT_FILTER is None else args.PROJECT_FILTER

    print(search(gitlab_server_arg, token_arg, file_filter_arg, text_arg, group_arg, project_filter_arg, api_debug_arg,
                 internal_debug_arg, regex_arg))
